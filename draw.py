...
def scanline_convert(polygons, i, screen, zbuffer ):
    ...
    triangles = polygons[i:i+3]
    maxy = max([triangles[x][1] for x in range(3)])
    miny = min([triangles[x][1] for x in range(3)])
    for i in range(3):
        if triangles[i][1] == maxy:
            topi = i
            top = triangles[i]
        if triangles[i][1] == miny:
            boti = i
            bot = triangles[i]
    mid = triangles[3 - topi - boti]
    #print(top, mid, bot)


    xb, yb, zb = bot[0], bot[1], bot[2]
    xm, ym, zm = mid[0], mid[1], mid[2]
    xt, yt, zt = top[0], top[1], top[2]
